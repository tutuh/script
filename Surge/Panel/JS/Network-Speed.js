...
async function main() {
  try {
    ...
    const speedMbps = duration > 0 ? round((mb / duration) * 8) : 0
    ...
    const iconMap = {
      '1': (arg && arg.iconslow) || 'tortoise',
      '2': (arg && arg.iconmid) || 'hare',
      '3': (arg && arg.iconfast) || 'bolt'
    }

    const colorMap = {
      '1': (arg && arg.colorlow) || '#9E9E9E',
      '2': (arg && arg.colormid) || '#4CAF50',
      '3': (arg && arg.colorhigh) || '#FFC107'
    }

    icon = iconMap[tier]
    color = colorMap[tier]

    console.log(`网速档位: ${tier}`)
    console.log(`图标: ${icon}`)
    console.log(`颜色: ${color}`)
    console.log(`延迟: ${pingt}ms`)
    console.log(`耗时: ${round(duration, 2)}s`)

    title = arg.title || 'NetSpeed'

...
!(async () => {
  ...
  // 1. 先测延迟 (Ping)，避免下载测速占用带宽导致 Bufferbloat 影响延迟数据
  const pingstart = Date.now()
  await $.http.get({
    url: `http://cp.cloudflare.com/generate_204`
  }).catch(() => {}) // 忽略单次 Ping 的抛错，以免中断主测速逻辑
  ...
  const speedMbps = Math.round((mb / duration) * 8) // 转为 Mbps 并四舍五入到整数

  console.log(`延迟: ${pingt}ms`)
  console.log(`耗时: ${duration.toFixed(2)}s`)
  console.log(`下行速率: ${speedMbps} Mbps`)

  // 3. 确定网速档位 (1: <30Mbps, 2: 30~100Mbps, 3: >100Mbps)
  let tier = 1; 
  if (speedMbps > 100) {
    tier = 3;
  ...
  console.log(`网速档位: ${tier}, 图标: ${icon}, 颜色: ${color}`)

...
$.isPanel = () => $.isSurge() && typeof $input !== 'undefined' && $input?.purpose === 'panel'
$.isTile = () => $.isStash() && typeof $script !== 'undefined' && $script?.type === 'tile'
...
let content = ''
// 默认图标和颜色（网络未加载或出错时显示）
let dynamicIcon = arg.icon || 'network'
let dynamicColor = arg.iconColor || arg['icon-color'] || '#3498db'

const round = (num, precision = 2) => Number(`${Math.round(`${num}e${precision}`)}e-${precision}`);

!(async () => {
  if ($.isTile()) {
    await notify(title, '面板', '开始查询')
  }
  
  const mb = Number(arg.mb) || 1
  ...
  const time = res?.headers?.['cf-meta-request-time']
  const end = Date.now()
  ...
  content = `${round(speedMbps)} Mbps\n${round(speed)} MB/s\n耗时: ${round(duration)}s\n执行时间: ${new Date().toTimeString().split(' ')[0]}`
  
  if ($.isTile()) {
    await notify(title, '面板', '查询完成')
  } else if (!$.isPanel()) {
  ...
  .catch(async e => {
    $.logErr(e)
    title = '❌'
    content = e?.message || e?.error || String(e)
    dynamicIcon = 'exclamationmark.triangle.fill'
    dynamicColor = '#e74c3c'
    await notify('网络速率', title, content)
  })
  .finally(() => {
    // 将动态生成的 icon 和 icon-color 注入结果中，覆盖原本引用的静态 arg 
    const result = { 
      title, 
      content, 
      ...arg, 

...
!(async () => {
  ...
  try {
    ...
    let end = Date.now()
    duration = Math.max((end - start) / 1000, 0.001) // 避免因运行过快分母为 0 导致 NaN
    speed = mb / duration
  } catch(e) {
    $.logErr('下行测速失败: ' + e.message)
  }
  ...
  try {
    ...
    $.logErr('Ping 测试失败: ' + e.message)
    pingt = 999 // 失败时赋予极高延迟值以触发正确的区间颜色
  }

  console.log('to see ping:' + pingt)

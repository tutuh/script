...
!(async () => {
  ...
  await $.http.get({
    ...
    timeout: 5000 // 增加 5 秒超时，防止断网时面板卡死
  });
  const end = Date.now();
  ...
  console.log(`耗时: ${duration}s | 延迟: ${pingt}ms`);

  // 3. 稳健的网速区间判断逻辑：1 (<30Mbps), 2 (30Mbps~100Mbps), 3 (>=100Mbps)
  let speedLevel = 1;
  if (speedMbps >= 100) {
    speedLevel = 3;
  ...
  const color = colors[speedLevel];

  console.log(`网速档位: ${speedLevel}, 图标: ${icon}, 颜色: ${color}`);

  const title = arg?.title || `NetSpeed`;
  ...
  // 正常执行完毕，将结果传递给下一个 then 处理
  return { title, content, icon, 'icon-color': color };
})()
.catch(async e => {
  $.logErr(e);
  const msg = e.message || String(e);
  const errTitle = '❌ 测速失败';
  await notify('网络速率', errTitle, msg);
  
  // 发生异常时，返回红色警告 UI 给下一个 then 处理
  return { 
    title: errTitle, 
    content: msg, 
...
.then(result => {
  // 统一在此处合并参数并调用 $.done()，避免在 catch 和 finally 中被调用两次
  const finalResult = { ...arg, ...result };
  $.log(JSON.stringify(finalResult));
  $.done(finalResult);
});

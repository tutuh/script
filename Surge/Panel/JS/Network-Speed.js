...
!(async () => {
  ...
  let speedMbps = 0
  let duration = 0
  let isSpeedTimeout = false

  // 1. Ping 测试 (设置 3000ms 超时限制)
  ...
  try {
    ...
    pingt = -1 // -1 代表超时或失败
    $.log("Ping 失败或超时: " + (e.message || e))
  }

  // 2. 下载测速 (设置 8000ms 超时限制，防止触发代理软件强制杀后台)
  const mb = Number($.lodash_get(arg, 'mb')) || 1
  ...
  try {
    ...
    speedMbps = Math.round((mb / duration) * 8)
  } catch (e) {
    isSpeedTimeout = true
    duration = (Date.now() - start) / 1000
    $.log("下载测速失败或超时: " + (e.message || e))
  }

  console.log(`延迟: ${pingt}ms, 耗时: ${duration.toFixed(2)}s, 速率: ${speedMbps} Mbps`)

  // 3. 确定网速档位 (1: <30Mbps或超时, 2: 30~100Mbps, 3: >100Mbps)
  let tier = 1; 
  if (speedMbps > 100) {
    tier = 3;
  ...
  const icons = { 1: arg?.iconslow, 2: arg?.iconmid, 3: arg?.iconfast }
  const colors = { 1: arg?.colorlow, 2: arg?.colormid, 3: arg?.colorhigh }
  
  icon = icons[tier] || ''
  color = colors[tier] || ''

  // 4. 构建面板显示文本 (容错处理)
  const pingStr = pingt === -1 ? '超时断流' : `${pingt} ms`
  const speedStr = isSpeedTimeout ? '测速超时' : `${speedMbps} Mbps`

  title = `NetSpeed`
  content = `下行速率: ${speedStr}\n网络延迟: ${pingStr}\n测试耗时: ${duration.toFixed(2)}s`

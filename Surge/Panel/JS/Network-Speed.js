...
!(async () => {
  ...
  const speedMbps = round(Math.abs(speed * 8))
  ...
  // 区间判定：[0, 30, 100] 对应三个档位
  // 返回值：1 (<30Mbps), 2 (30Mbps~100Mbps), 3 (>100Mbps)
  const a = Diydecide(0, 30, 100, speedMbps)
  
  // 延迟区间判定
  const b = Diydecide(0, 80, 150, pingt)
  ...
  // 让图标和颜色同时匹配网速区间(a)
  icon = shifts[String(a)]
  color = shifts[String(parseInt(a) + 3)]

  console.log(`网速档位: ${a}, 图标: ${icon}, 颜色: ${color}`)
...
// 确定变量所在区间 (已修复 JavaScript 默认字符串排序的 Bug)
function Diydecide(x, y, z, item) {
  let array = [x, y, z]
  array.push(item)
  // 必须传入显式的数值比较函数 (a, b) => a - b

...
async function main() {
  ...
  for (const bar of bars) {
    ...
    if (bar.is_sign == 1) {
      ...
      result.push(
        `【${bar.forum_name}】已经签到，等级${bar.user_level}，经验${bar.user_exp}`
      );
    ...
    if (r.success) {
      ...
      result.push(
        `【${bar.forum_name}】签到成功，${r.msg}`
      );

    } else {

      result.push(
        `【${bar.forum_name}】签到失败：${r.msg}`
      );

    }

    console.log(
      `${bar.forum_name}: ${r.msg}，等待 ${(wait / 1000).toFixed(2)} 秒`
    );
  }
...
async function sign(kw, tbs) {
  ...
  for (let attempt = 1; attempt <= CONFIG.retry; attempt++) {
    ...
    console.log(
      `${kw} 第${attempt}/${CONFIG.retry}次签到失败：${lastMsg}`
    );
    ...
    if (attempt < CONFIG.retry) {
      ...
      console.log(
        `${kw} 等待 ${(delay / 1000).toFixed(1)} 秒后重试`
      );
  ...
  return {
    success: false,
    msg: `重试${CONFIG.retry}次失败：${lastMsg}`
  };
}
    ...
    $httpClient.post(
      {
        ...
        body:
          `tbs=${tbs}&kw=${encodeURIComponent(kw)}&ie=utf-8`
      },
        ...
        try {
          ...
          if (obj.no == 0) {
            ...
            resolve({
              success: true,
              msg:
                `连续签到${obj.data.uinfo.cont_sign_num}天，排名${obj.data.uinfo.user_sign_rank}`
            });
